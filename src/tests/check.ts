...
describe('#Check', () => {
  ...
  it('Should return correct result object', () => {
    assert.hasAllKeys(createGameSettingsConfig(readJSONFileSync(`${process.cwd()}/settings.json`)), ['newUserMessages', 'newSettingsConfigObj']);
  });

  it('Should return no messages array', () => {
    assert.equal(createGameSettingsConfig(readJSONFileSync(`${process.cwd()}/settings.json`)).newUserMessages.length, 0);
  });
  ...
  it('Should return array with messages', () => {
    ...
    assert.equal(createGameSettingsConfig(obj).newUserMessages.length, 1);
    // @ts-ignore
  ...
  it('Should return array with error message', () => {
    ...
    // @ts-ignore
    delete obj.usedFiles;
    assert.equal(createGameSettingsConfig(obj).newUserMessages[0].type, 'error');
  });
  ...
  it('Should return array with warning message', () => {
    ...
    // @ts-ignore
    obj.newField = 'new';
    assert.equal(createGameSettingsConfig(obj).newUserMessages[0].type, 'warning');
  });
  ...
  it('Should return array with info message', () => {
    ...
    assert.equal(createGameSettingsConfig(obj).newUserMessages[0].type, 'info');
  });
});
