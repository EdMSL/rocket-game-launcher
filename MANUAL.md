...
1. *Чекбокс (checkbox)*.
 ...
 ![checkbox](https://i.imgur.com/MiTO7lh.jpeg).
...
2. *Переключатель (switcher)*.
 ...
 ![switcher](https://i.imgur.com/IjdtS4i.jpeg).
...
![range](https://i.imgur.com/ATBepLs.jpeg). 
...
Если значением опции является число с плавающей запятой, то кол-во знаков после запятой определяется количеством этих знаков из исходного значения параметра из конфигурационного файла этого параметра.

4. *Выпадающий список (select)*.

![select](https://i.imgur.com/0l7b9WI.jpeg).
...
Представляет собой массив, каждый элемент которого объект с описанием опции. Опции бывают **четырех** типов: *обычные*, *связанные*, *комбинированные* и *групповые*. От типа опции зависит кол-во регулируемых параметров в файле, а так же кол-во UI контроллеров для опции и доступный их тип.
...
Некоторые поля являются общими для всех типов опций, другие же уникальны. Ввиду большого кол-ва нюансов, каждый тип опций будет разобран отдельно.
