...
## Рекомендации
Для комфортной работы с конфигурационными файлами программы, которые имеют расширение `.json`, настоятельно рекомендуется использовать текстовые редакторы с подсветкой синтаксиса, например, NotePad++. Так как синтаксис этого формата строгий, это убережет от синтаксических ошибок на этапе изменения файлов. От себя лично рекомендую использовать [VS Code](https://code.visualstudio.com/) - текстовый редактор на все случаи жизни.

## Интерфейс

Интерфейс программы состоит из двух основных экранов: *Главный экран* и *Экран игровых настроек*.

Экран игровых настроек содержит панель навигации по группам настроек, список опций, переключатель профилей Mod Organizer (если МО использутся) и кнопки управления настройками.

### Главный экран

![Главный экран](https://live.staticflickr.com/65535/51536044535_9d21825e1b_o.jpg)

На главном экране располагается панель навигации, где отображаются кнопка запуска игры (**1**), кнопка настроек программы (**4**), кнопка перехода на экран настроек игры (**3**), если настройки были добавлены в файл игровых настроек, и пользовательские кнопки (**2**), если они были добавлены в файл конфигурации программы (о файлах игровых настроек и конфигурации программы информация ниже).

Помимо этого в заголовке окна програамы есть блок (**5**), в котором указано имя игры\программы, для которой предназначена программа. Отображаеиый текст можно задать в конфигурационном файле программы.

Кнопка "Играть" запускает игру\приложение. Вы можете настроить путь к приложению и заголовок кнопки. При нажатии на кнопку "Настройки" будет произведен переход на экран игровых настроек. Если созданы группы игровых настроек, откроется группа, записанная первой в списке. Кнопка настроек программы (по умолчанию содержит значек шестеренки) откроет окно настроек программы. Доступные настройки следующие:
...
![Экран настроек](https://live.staticflickr.com/65535/51562363510_4e76453b60_o.jpg)
...
Наигация по группам настроек отображает вкладки для перехода между группами, если группы настроек были созданы. Иначе здесь будет только кнопка "Назад" для перехода на главный экран.

Переключатель МО изменяет текущий профиль. Данные записываются в файл `ModOrganizer.ini`. При изменении профиля обновляются значения опций, которые основаны на параметрах из файлов, располагающихся в папке текущего профиля МО.

Список опций содержит контроллеры для управления опциями. Информация о типах контроллеров в соответствующем разделе ниже.

Блок кнопок управления состоит из четырех кнопок:
* "Сохранить". Записывает изменения параметров в файл.
* "Сбросить". Сбрасывает значения измененнных опций, возвращая их к тем, что были у параметров после первой загрузки. При сохранении изменений стандартное значение изменится на сохраненное, и сброс будет осуществляться на это новое значение.
* "Обновить". Перезагружает окно настроек и перечитывает данные из конфигурационного файла `settings.json`, тем самым обновляя список опций и ихх значений, а так же профили Mod Organizer.
* "Создать бэкап". Создает бэкап файлов, параметры из которых использованы в опциях.
* "Бэкапы". Открывает окно бэкапов.

### Окно бэкапов

![Окно бэкапов](https://live.staticflickr.com/65535/51562369710_8a1106b119_o.jpg)

В программе предусмотрена возможность создания бэкапов файлов, параметры из которых использованы в опциях. Проще говоря, это все файлы-элементы объекта ***gameSettingsFiles*** из файла конфигурации игровых настроек `settings.json`. Бэкапы создаются по кнопке "Создать бэкап" на экране игровых настроек программы. Помимо этого бэкап можно создать из окна "Бэкапы", кнопка к которму расположена рядом с кнопкой "Создать бэкап" на экране настроек.
В верхней части окна "Бэкапы" будет отображен список доступных бэкапов или сообщение об их отсутствии (**1**). Рядом с именем бэкапа находится кнопки переименования и удаления данного бэкапа. Каждый элемент списка представляет собой раскрывающийся список, который содержит файлы данного бэкапа (**2**). Вы можете выбрать все или несколько файлов и восстановить их из бэкапа, нажав кнопку "Восстановить выбранное". Если нажать на путь к файлу, строка которого расположена под заголовком файла бэкапа, то откроется папка, где расположен оригинальный файл. Ниже расположен блок кнопок. "Создать бэкап" создает новый бэкап файлов, "Обновить" обновляет список доступных бэкапов. "Открыть папку с бэкапами" откроет папку с *game_settings_files*, расположенную в папке *backups*, которая располагается к корневой папке программы и хранит все бэкапы. Кнопка "Отмена" закроет окно бэкапов.
...
Общая информация об ошибке выводится в правой нижней части окна программы. Более подробную информацию об ошибке можно посмотреть в файде лога `launcher.log`, который находится в корневой папке программы.
