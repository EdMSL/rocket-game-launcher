...
## Краткое описание
Программа предназначена для запуска игр и настройки параметров из конфигурационных файлов игры. В первую очередь программа интересна тем, кто занимается разработкой сборок на основе какой-либо игры. Разрабатывалась для сборки **Oblivion Association** на основе **TES4: Oblivion**, затем была доработана для использования с другими играми. Присутствует поддержка Mod Organizer 1 и 2.
...
- Управление параметрами из конфигупационных `.ini` и `.xml` файлов с помощью UI элементов (селектов, ползунков и т.д.).
...
```js
{
  ...
  // Игнорируется, если `isResizable` в значении false.
  // Если `isResizable` = false, то указывать следует равным `width`. Иначе при нажатии *F11* окно будет разворачиваться на весь экран (особенность платформы).
  // Тип: число.
  // По умолчанию: 0 (нет ограничений).
  "maxWidth": 0,
...
Рассмотрим на примере Mod Organizer. При его использовании игру следует запускать через него, иначе игра не увидит установленные плагины. Для облегчения запуска игры и приложений, работающих с модами, в МО можно создать ярлыки.
